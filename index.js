...
client.on("messageCreate", async message => {
  ...
  /* ========== WELCOME ON POINT "." ========== */
  if (message.content === ".") {
    message.channel.send(`Welcome ${message.author} to Gaming Community! 🎉`);
  }
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.customId === "ticket") {
    ...
    const btn = new ButtonBuilder().setCustomId("close").setLabel("Close").setStyle(ButtonStyle.Danger);
    const row = new ActionRowBuilder().addComponents(btn);
  ...
  if (action === "kick") {
    channel.members.forEach(m => { if (m.id !== interaction.user.id) m.voice.disconnect(); });
